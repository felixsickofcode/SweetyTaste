...
void Player::Audio(AudioManager & audio) {
    if (on_ground && inp_type.jump == 1) audio.playSound("jump");
    Uint32 now = SDL_GetTicks();
    if (on_ground && actionState == RUNNING) {
        Uint32 now = SDL_GetTicks();
    ...
    if (actionState == ATTACKING) {
        int f = frame[actionState];
        if ((f == 4 || f == 8 || f == 12 || f == 18)) {
            if (f != lastAttackFrame)
                audio.playSound("attack");
            lastAttackFrame = f;
        }
    } else lastAttackFrame = -1;
...
void Player::DoPlayer(MapObject & map_data, MapObject & visual_map) {
    ...
    if (inp_type.attack == 1) {
        actionState = ActionState::ATTACKING;
        if (inp_type.right == 1) x_val += 1.2 * PlayerSpeed;
        else if (inp_type.left == 1) x_val -= 1.2 * PlayerSpeed;
        attack_input_timer.Start();
        inp_type.attack = 0;
    }
    if (actionState == ATTACKING) {
        UpdateRepeatFrame(20, attack_timer, frame_delay);
...
void Player::HandleInput(SDL_Event event) {
    ...
    if (event.type == SDL_KEYDOWN && event.key.repeat == 0) {
        ...
        case SDLK_SPACE:
            if (on_ground == 1)
                inp_type.jump = 1, jump_input_timer.Start();
            break;
        case SDLK_j:
            inp_type.attack = 1;
            break;
